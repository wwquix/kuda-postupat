...
if TYPE_CHECKING:
    from .catalog_models import DataSource, LegacySpecialtyMapping, ProgramOffering
...
class Specialty(Base):
    ...
    catalog_mapping: Mapped[LegacySpecialtyMapping | None] = relationship(
        back_populates="legacy_specialty", uselist=False, passive_deletes=True
    )
...
class AdmissionSnapshot(Base):
    ...
    __table_args__ = (
        Index("ix_admission_snapshots_program_offering_id_fetched_at", "program_offering_id", "fetched_at"),
    )
    ...
    program_offering_id: Mapped[int | None] = mapped_column(
        ForeignKey("program_offerings.id", ondelete="RESTRICT"), nullable=True
    )
    specialty: Mapped[Specialty] = relationship(back_populates="snapshots")
    program_offering: Mapped[ProgramOffering | None] = relationship(back_populates="legacy_snapshots")


class ScraperRun(Base):
    __tablename__ = "scraper_runs"
    __table_args__ = (Index("ix_scraper_runs_data_source_id_started_at", "data_source_id", "started_at"),)
    ...
    data_source: Mapped[DataSource | None] = relationship(back_populates="legacy_scraper_runs")
