...
function SearchControl({ value, onCommit }: { value: string; onCommit: (value: string) => void }) {
  ...
  return <form
    ...
    className="surface-sunken flex min-h-11 min-w-0 flex-1 items-center gap-2 bg-elevated px-2"
    ...
    role="search"
  >
    <Search aria-hidden="true" className="ml-2 shrink-0 text-accent" size={20} />
    <label className="sr-only" htmlFor="university-search">Поиск по названию, городу или категории</label>
    <input
      autoComplete="off"
      className="min-w-0 flex-1 bg-transparent px-1 py-2 text-base outline-none placeholder:text-text-tertiary"
    ...
    {draft && <button
      ...
      className="grid min-h-11 min-w-11 place-items-center rounded-xl text-text-tertiary"
      onClick={() => {
        setDraft('')
        commit('')
      }}
...
function FilterSelect({ id, label, value, onChange, children }: FilterSelectProps) {
  return <label className="grid gap-2 text-sm font-semibold text-text-secondary" htmlFor={id}>
    {label}
    <span className="relative">
      <select
        className="field-control w-full appearance-none pr-9 font-normal"
        id={id}
        onChange={(event) => onChange(event.target.value)}
        value={value}
      >{children}</select>
      <ChevronDown aria-hidden="true" className="pointer-events-none absolute right-3 top-1/2 -translate-y-1/2 text-text-tertiary" size={16} />
    </span>
  </label>
}
...
function UniversityFilters({
  ...
  return <details
    ...
    open={open}
  >
    <summary className="flex min-h-11 cursor-pointer list-none items-center justify-between gap-3 rounded-xl font-bold lg:hidden">
      <span className="flex items-center gap-2"><Filter aria-hidden="true" size={18} />Фильтры</span>
      <ChevronDown aria-hidden="true" className="transition-transform duration-200 group-open:rotate-180 [transition-timing-function:var(--ease-out)]" size={18} />
    </summary>
...
function UniversityCard({ university, returnTo }: { university: UniversityListItem; returnTo: string }) {
  ...
  return <article className="panel flex min-w-0 flex-col gap-4 p-5">
    <div className="flex min-w-0 flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
      <div className="min-w-0">
        ...
        <h2 className="mt-2 break-words text-xl font-semibold leading-snug">
          <Link
            className="inline-block rounded-sm text-text-primary underline decoration-accent/35 decoration-2 underline-offset-4 hover:decoration-accent"
      ...
      <span className="w-fit shrink-0 rounded-full border border-text-primary/10 bg-background px-3 py-1.5 text-xs font-bold text-text-secondary">
        {monitoringLabel}
      </span>
    </div>
    ...
    <div className="flex flex-wrap gap-2 text-xs font-semibold text-text-secondary">
      ...
      <span className="rounded-full bg-accent-soft px-3 py-1.5">{apiValueLabel(university.institution_kind)}</span>
      {university.categories.map((category) => <span className="rounded-full border border-accent/15 px-3 py-1.5" key={category.code}>{category.label_ru}</span>)}
    </div>

    <div className="grid gap-3 sm:grid-cols-2">
      <div className="surface-sunken p-4">
        <div className="text-xs font-bold uppercase tracking-wider text-text-tertiary">Покрытие платформы</div>
      ...
      <div className="surface-sunken p-4">
        <div className="text-xs font-bold uppercase tracking-wider text-text-tertiary">Для абитуриентов</div>
    ...
    <div className="mt-auto flex flex-wrap items-center gap-4 border-t border-text-primary/10 pt-4 text-sm">
      <a
      ...
      {hasLiveMonitoring && <Link className="inline-flex items-center gap-1.5 font-semibold text-accent hover:underline" to="/monitor">
        <RadioTower aria-hidden="true" size={16} />Live-мониторинг БГЭУ
      </Link>}
      <Link className="inline-flex items-center gap-1.5 font-semibold text-accent hover:underline" to={`/compare?universities=${encodeURIComponent(university.slug)}`}>Сравнить</Link>
...
function CatalogPagination({
  ...
  return <nav aria-label="Страницы каталога" className="mt-8 flex flex-wrap items-center justify-center gap-2">
    <button
      className="button-secondary min-h-11 px-3 text-sm disabled:cursor-not-allowed disabled:opacity-45"
    ...
    {compactPageNumbers(pagination.page, pagination.total_pages).map((page) => typeof page === 'number'
      ? <button
        ...
        className={`min-h-11 min-w-11 rounded-xl border px-3 py-2 text-sm font-bold ${page === pagination.page ? 'border-accent bg-accent text-white' : 'border-border bg-white text-text-primary'}`}
        key={page}
        onClick={() => onPage(page)}
        type="button"
      >{page}</button>
      : <span aria-hidden="true" className="px-1 text-text-tertiary" key={page}>…</span>)}
    <button
      className="button-secondary min-h-11 px-3 text-sm disabled:cursor-not-allowed disabled:opacity-45"
...
function LoadingCards() {
  return <div aria-live="polite" className="grid gap-5 lg:grid-cols-2" role="status">
    ...
    {[1, 2, 3, 4].map((item) => <div aria-hidden="true" className="panel min-h-72 bg-white/70 p-6" key={item}>
      <div className="h-3 w-20 rounded bg-text-primary/10" />
      <div className="mt-4 h-7 w-4/5 rounded bg-text-primary/10" />
      <div className="mt-3 h-4 w-2/5 rounded bg-text-primary/10" />
      <div className="mt-8 h-24 rounded-2xl bg-text-primary/5" />
    </div>)}
  </div>
}
  ...
  return <div className="page-shell">
    <div className="page-container py-10 lg:py-14">
      <header className="max-w-3xl">
        ...
        <h1 className="page-heading">Вузы Беларуси</h1>
        <p className="body-copy mt-4">Ищите по импортированному каталогу и уточняйте сведения на официальных сайтах вузов. Покрытие программ и мониторинга пока неполное.</p>
      </header>

      <div className="glass-surface--compact mt-8 flex flex-col gap-2 p-2.5 lg:flex-row lg:items-center">
        ...
        <div className="flex shrink-0 flex-wrap items-center gap-2">
          ...
          <span className="relative min-w-48 flex-1 lg:flex-none">
            <SlidersHorizontal aria-hidden="true" className="pointer-events-none absolute left-3 top-1/2 -translate-y-1/2 text-accent" size={17} />
            <select
              className="field-control w-full appearance-none py-3 pl-10 pr-9 text-sm font-semibold"
              id="catalog-sort"
          ...
          <select
            className="field-control px-3 py-3 text-sm font-semibold"
            id="catalog-order"
      ...
      <div className="mt-5">
        {metaLoading && <div aria-live="polite" className="panel p-5 text-sm text-text-secondary" role="status">Загружаем параметры каталога…</div>}
        {metaError && <div aria-live="assertive" className="panel border-danger/30 p-6" role="alert">
          <h2 className="text-xl font-bold">Не удалось загрузить каталог</h2>
          <p className="mt-2 text-text-secondary">Сервис временно недоступен. Данные не заменены пустым списком.</p>
          <button className="mt-4 inline-flex items-center gap-2 rounded-xl bg-accent px-4 py-2.5 font-bold text-white" onClick={() => {
            setMetaLoading(true)
            setMetaError(false)
      ...
      {activeFilters.length > 0 && <section aria-label="Активные фильтры" className="mt-5 flex flex-wrap items-center gap-2">
        {activeFilters.map((filterItem) => <button
          ...
          className="inline-flex items-center gap-1.5 rounded-full border border-accent/20 bg-accent-soft px-3 py-1.5 text-sm font-semibold text-accent"
          key={filterItem.key}
          onClick={() => updateQuery(filterItem.patch)}
          type="button"
        >{filterItem.text}<X aria-hidden="true" size={14} /></button>)}
        <button className="rounded-xl px-3 py-1.5 text-sm font-bold text-accent hover:bg-accent/5" onClick={clearFilters} type="button">Очистить поиск и фильтры</button>
      </section>}

      <section aria-busy={results.loading} aria-labelledby="catalog-results-title" className="mt-8 min-w-0">
        <div className="mb-5 flex flex-col gap-2 sm:flex-row sm:items-end sm:justify-between">
          <div>
            <h2 className="text-2xl font-semibold" id="catalog-results-title">Результаты</h2>
            {results.data && !results.error && <p className="mt-1 text-sm text-text-secondary">Найдено в каталоге платформы: <strong>{results.data.pagination.total_items}</strong></p>}
          </div>
          {results.data && !results.error && <p className="text-sm font-semibold text-text-secondary">Страница {results.data.pagination.page} из {Math.max(1, results.data.pagination.total_pages)}</p>}
        </div>

        {meta && results.loading && !results.data && !results.error && <LoadingCards />}
        {meta && results.loading && results.data && !results.error && <div aria-live="polite" className="mb-4 inline-flex items-center gap-2 rounded-full bg-accent-soft px-3 py-1.5 text-sm font-semibold text-accent" role="status"><RefreshCw aria-hidden="true" className="animate-spin" size={15} />Обновляем результаты…</div>}
        {meta && results.error && <div aria-live="assertive" className="panel border-danger/30 p-6" role="alert">
          <h2 className="text-xl font-bold">Не удалось получить результаты</h2>
          <p className="mt-2 text-text-secondary">Попробуйте повторить запрос. Ошибка не означает, что подходящих вузов нет.</p>
          <button className="mt-4 inline-flex items-center gap-2 rounded-xl bg-accent px-4 py-2.5 font-bold text-white" onClick={() => {
            setResults((current) => ({ ...current, loading: true, error: false }))
        ...
        {meta && results.data && !results.error && results.data.items.length === 0 && !results.loading && <div className="panel p-7 text-center">
          ...
          <p className="mx-auto mt-3 max-w-xl text-text-secondary">Попробуйте изменить запрос или снять фильтры. Это состояние относится только к импортированному каталогу платформы.</p>
          {hasUniversityCatalogFilters(query) && <button className="mt-5 rounded-xl bg-accent px-5 py-3 font-bold text-white" onClick={clearFilters} type="button">Очистить поиск и фильтры</button>}
        </div>}
      ...
      <aside className="mt-10 rounded-3xl border border-warning/30 bg-warning/10 p-6 text-warning/85">
        <div className="flex items-start gap-3">
          <CheckCircle2 aria-hidden="true" className="mt-0.5 shrink-0 text-warning" size={20} />
